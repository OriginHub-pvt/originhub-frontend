...
export default function IdeaCard({ idea }: IdeaCardProps) {
  ...
  return (
    <Link
      href={`/ideas/${idea.id}`}
      className="group block rounded-xl border border-slate-200 bg-white p-6 shadow-sm transition-all hover:border-[#14b8a6] hover:shadow-lg"
    >
      {/* Header */}
      <div className="flex items-start justify-between">
        <div className="flex-1">
          <h3 className="text-lg font-semibold text-slate-900 group-hover:text-[#0e3a5f] transition-colors">
            {idea.title}
          </h3>
        ...
        <span
          className={`ml-4 rounded-full px-3 py-1 text-xs font-medium capitalize ${statusColors[idea.status]}`}
        >
          {idea.status}
        </span>
      </div>
